...
int main(int argc, char **argv)
{
    ...
    if (block[P2_I_MAGIC] != P3_MAGIC)
        goto publish;
    if (block[P2_I_VERSION] != P3_VERSION)
        goto publish;
    validated = 1;
    ...
    for (poll = 0; poll < 600000UL; ++poll) {
        ...
        if (block[P2_I_HOST_DONE] == P3_HOST_DONE)
            break;
    ...
    for (batch = 0; batch < batches; ++batch) {
        ...
        lastFence = sequence;
        ++submits;
        /* Pace the run: a compute delay per batch stretches the whole run
         * past the 5 s expiry, so only the host's heartbeats can keep the
         * lease alive. */
        {
            volatile ULONG sink = 0;

            for (index = 0; index < P3_PACE_SPINS; ++index)
                sink ^= index;
            (void)sink;
        }
    }
...
publish:
    ...
    if (result == 20)
        printf("PPCPHASE3 hint=usage: ppcphase2 <hexAddress>\n");
    return result;
}
